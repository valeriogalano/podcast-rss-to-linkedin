...
class TestFetchLastEpisode:
    ...
    def test_raises_on_missing_title(self):
        rss = b"""<?xml version="1.0"?><rss><channel><item><link>https://x.com</link></item></channel></rss>"""
        mock_resp = MagicMock()
...
class TestPublishToLinkedin:
    def test_sends_correct_request(self):
        episode = {"title": "Test Ep", "link": "https://ex.com/ep"}
        template = "Nuovo: {title}\n{link}"
        mock_resp = MagicMock()
        mock_resp.status_code = 201
        with patch("publish.requests.post", return_value=mock_resp) as mock_post:
            publish_to_linkedin(episode, "ACCESS_TOKEN", "person123", template)
        mock_post.assert_called_once()
        call_kwargs = mock_post.call_args
        assert "Bearer ACCESS_TOKEN" in call_kwargs[1]["headers"]["Authorization"]
        body = call_kwargs[1]["json"]
        assert "urn:li:person:person123" == body["author"]
    ...
    def test_raises_on_api_error(self):
        ...
        with patch("publish.requests.post", return_value=mock_resp):
            with pytest.raises(Exception, match="Errore LinkedIn API"):
                publish_to_linkedin(episode, "BAD_TOKEN", "person123", "{title}")
